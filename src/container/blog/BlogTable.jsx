...
const BlogTable = ({
  ...
  const { allBlogs, allSubCategory, searchText, blog } = useSelector(
    (state) => state.data
  );
  ...
  const handleDelete = async (id) => {
    try {
      await dispatch(deleteMultipleBlogs([id])).unwrap();
  ...
  useEffect(() => {
    console.log(searchText, "searasdfa");
  ...
  const fetchDataById = async (id, type) => {
    try {
      const result = await dispatch(fetchBlogByid(id)).unwrap(); // this will be the current blog data
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      const allIds = allBlogs?.data?.map((item) => item?.id);
