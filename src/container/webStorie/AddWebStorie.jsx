...
export default function AddWebStorie({
  isOpen,
  setIsOpen,
  webStoriesData,
  setwebStoriesData,
  ...
  useEffect(() => {
    if (webStoriesData) {
      setFormData({
        image: webStoriesData.image || "",
        heading: webStoriesData.heading || "",
        description: webStoriesData.description || "",
        category: webStoriesData.category || "",
      });
    }
  }, [webStoriesData]);
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (webStoriesData) {
        // Update blog
        await dispatch(
          updateWebStories({ id: webStoriesData?.id, data: payload })
        ).unwrap();
        await dispatch(fetchWebStories({ search: "" })).unwrap();
        setwebStoriesData(null);
      } else {
        // Add new blog
        await dispatch(insertWebStories(payload)).unwrap();
        await dispatch(fetchWebStories({ search: "" })).unwrap();
      }
      // Reset form
      setFormData({
  ...
  const handleClose = () => {
    setIsOpen(false);
    if (webStoriesData) setwebStoriesData(null);
  };
  return (
    <>
      <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white w-[90%] max-w-md p-6 rounded-lg shadow-lg relative">
            ...
            <h2 className="text-lg font-bold text-gray-800 mb-4">
              {webStoriesData ? "Update Blog" : "Save Blog"}
            </h2>
